...
def main() -> None:
    ...
    if optionalArg["name"] == "output" and optionalArg["value"] == "log":
        ...
        for idx, engine in enumerate(ENGINES):
            engine_mems = filterData(engine, mems, table_sizes)
            engine_times = filterData(engine, times, table_sizes)
...
def filterData(engine, set, table_sizes=None):
    ...
    if table_sizes is None:
        ...
        for elt in set:
            if engine.name() not in elt:
                return res
            res.append(elt[engine.name()])
        return res
    for i in range(len(table_sizes)):
        if engine.name() in set[i]:
            res.append(set[i][engine.name()])
        else:
            res.append(np.nan)
    return res
